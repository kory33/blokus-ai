...
class SelectiveDQNPolicy<O : Encodable, AS : SelectiveDiscreteSpace>(private val mdp : MDP<O, Int, AS>,
    ...
    private fun filterQOutPut(qs: INDArray): INDArray {
        val filteringVector = mdp.actionSpace.computeActionAvailability()
        return qs.muliRowVector(filteringVector)
    }

    override fun nextAction(input: INDArray): Int {
        val output = dqn.output(input)
        return Learning.getMaxAction(filterQOutPut(output))
    }
}